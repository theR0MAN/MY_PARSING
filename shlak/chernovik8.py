		...
		for i in symbols:
			...
			if "RTS\\FORTS\\" in i['path'] and 'Expired' not in i['path'] and 'Splice' not in i['name']:  # Expired
				if mt5.market_book_add(i['name']):
					a[i['name']] = dict()
					a[i['name']]["-1"] = dict() # буфер для фикса изменений -удалить перед записью
		time.sleep(3)  # на mt5.market_book_add нужно время, операция асинхронная- поэтому повторно ниже дублируем цикл
		 #запись первого
