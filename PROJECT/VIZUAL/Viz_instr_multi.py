...
stop_year, stop_month, stop_day, stop_hour = 	 2023, 9, 22, 13
fixkf=1
...
def get_content(content):
	...
	lsp = int(ln / yadra)
	hvost = ln - lsp * yadra
	hvostsp = content[ln - hvost:]
	c2 = list(zip(*[iter(content)] * yadra))
	c2.append(tuple(hvostsp))
	return c2
...
if __name__ == '__main__':
	...
	def f_proc(part_content):
		# print(f'part_content ={part_content}')
		with Pool(processes=yadra) as pool:
			results = pool.map(Func,part_content)
		return results

	# цикл перебираем части конт
	c2= get_content(content)
	data = dict()
	nomfile = -1
	for cont in c2 :
		# всандаливаем в распараллел
		rez = f_proc(cont)
		# print(f'results ={rez}')

	# 	а теперь перебераем поштучно
							...
							for tmp in range(stper):
								...
								if str(tmp) in a[inst]:
									if "kf" not in data[inst]:
										try:
											kf = 200 / (a[inst][str(tmp)]['asks'][0][0] + a[inst][str(tmp)]['bids'][0][
												0])
										except:
	...
	print(f" 2 time= {time.time() - timer}")
	color = get_color()
	fig = px.line()
	for inst in data:
		clr = color()
