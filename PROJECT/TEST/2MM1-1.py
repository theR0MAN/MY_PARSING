	...
	for kperso in kpersos:
		for ksovh in ksovhs:
			for koefvih in koefvihs:
				...
				for sym in dats['Si']:
					mystruct[idev][sym] =[]
					# mystruct[idev][sym] = dict()
					# mystruct[idev][sym]['buy'] = []
					# mystruct[idev][sym]['buyvih'] = []
					# mystruct[idev][sym]['sell'] = []
					# mystruct[idev][sym]['sellvih'] =[]
	...
	try:
										...
										for ksovh in ksovhs:
											...
											for koefvih in koefvihs:
												...
												if Bidsp > SoAsk * ksovih and totrade[id0]:  # Bidsp>SoAsk
													...
													signals[idev][sym1]['buyvih'].append(sym2)
													signals[idev][sym2]['sellvih'].append(sym1)
												...
												if Bidsp > SoAsk * ksovh and So > minkso and totrade[id0]:  # and (So1-So2) >So/2:
													signals[idev][sym1]['sell'].append(sym2)
													signals[idev][sym2]['buy'].append(sym1)
													if flagsell[id]:
												...
												if Asksp < SoBid * ksovh and So > minkso and totrade[id0]:  # and (So1-So2) >So/2:
													signals[idev][sym1]['buy'].append(sym2)
													signals[idev][sym2]['sell'].append(sym1)
													if flagbuy[id]:
		...
		for idev in idevmas:
			for sym in idevmas[idev] :
				mystruct[idev][sym].append([dAsk[sym],dBid[sym], signals[idev][sym]])
