		...
		if sym1 in data and sym2 in data:
			if data[sym1]['dat'] != None and data[sym2]['dat'] != None:
				if data[sym1]['tmstp'][2] and data[sym2]['tmstp'][2]:
					...
					Ask1 = log(data[sym1]['dat'][0])
					Bid1 = log(data[sym1]['dat'][1] )
					Ask2 = log(data[sym2]['dat'][0] )
					Bid2 = log(data[sym2]['dat'][1] )


					Ask1f = log(futuredata[sym1]['dat'][0])
					Bid1f = log(futuredata[sym1]['dat'][1])
					Ask2f = log(futuredata[sym2]['dat'][0])
					Bid2f = log(futuredata[sym2]['dat'][1] )
					...
					if sredn1 != None and sredn2 != None:
						...
						if So != None:
							...
							if  flagbuyvih and Bidsp > SoAsk * ksovih  and totrade:  # Bidsp>SoAsk
							...
							if flagsellvih and Asksp < SoBid * ksovih and  totrade:  # Asksp<SoBid
								REZ.append(['sellvih',  [Ask1, Bid1], [Ask2, Bid2], [Ask1f, Bid1f], [Ask2f, Bid2f]])
								# timesellvih=tme
								...
								# print('ok')
							# покупка
							if flagbuy and Asksp < SoBid * ksovh and  So > minkso and totrade:  # and (So1-So2) >So/2:
								REZ.append(['buy',  [Ask1, Bid1], [Ask2, Bid2], [Ask1f, Bid1f], [Ask2f, Bid2f]])
								...
								# print('ok')
							# продажа
							if flagsell and Bidsp > SoAsk * ksovh and  So > minkso and totrade:  # and (So1-So2) >So/2:
								REZ.append(['sell',  [Ask1, Bid1], [Ask2, Bid2], [Ask1f, Bid1f], [Ask2f, Bid2f]])
...
if showspread:
	color = get_color()
	fig = px.line(
		title=f"4 n SPREAD {sym1} {sym2}   persr={persr}   kperso={kperso}  ksovh ={ksovh}  ksovih ={ksovih}   minkso={minkso}   ")
...
color = get_color()
fig = px.line(
	title=f"4 REZ {sym1} {sym2}  persr={persr}   kperso={kperso}  ksovh ={ksovh}  ksovih ={ksovih}   minkso={minkso}   zaderzka={zaderzka} ")
clr = color()
