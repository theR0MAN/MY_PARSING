...
start_year, start_month, start_day, start_hour = 2023, 10, 11, 12
stop_year, stop_month, stop_day, stop_hour = 	 2023, 10, 12, 18
...
while True:
	try:
		...
		for key in B:
			mascen = []
			for i in B[key]:
				if i in baza:  # вот оно че - в базе нет
					Ask = a[i]['asks'][0][0]
					Bid = a[i]['bids'][0][0]
