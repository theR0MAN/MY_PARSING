...
REZdict=dict()
dats = {'Si': ['SiM4*FRTS2', 'USDRUBF*FRTS2']}  # , 'SiU4*FRTS2' 'USD000UTSTOM*CUR'   'USDRUBF*FRTS2', 'SiU4*FRTS2'  , 'SiU4*FRTS2'
persrs = [1000]
	...
	try:
		...
		for onetwo in onetwomas:
			...
			if sym1 in data and sym2 in data:
				if data[sym1]['dat'] != None and data[sym2]['dat'] != None and data[sym3]['dat'] != None:
					if data[sym1]['tmstp'][2] and data[sym2]['tmstp'][2] and data[sym3]['tmstp'][2]:

						Ask1 = data[sym1]['dat'][0]
						Bid1 = data[sym1]['dat'][1]
						Ask2 = (data[sym2]['dat'][0]+data[sym3]['dat'][0])/2
						Bid2 = (data[sym2]['dat'][1]+data[sym2]['dat'][1])/2

						Ask1f = futuredata[sym1]['dat'][0]
						Bid1f = futuredata[sym1]['dat'][1]
						Ask2f = (futuredata[sym2]['dat'][0]+futuredata[sym2]['dat'][0])/2
						Bid2f = (futuredata[sym2]['dat'][1]+futuredata[sym2]['dat'][1])/2
...
for onetwo, persr, kperso, ksovh, koefvih in itertools.product(onetwomas, persrs, kpersos, ksovhs, koefvihs):
	...
	color = get_color()
	fig = px.line( title=f"D s REZ eq {id} ")
	clr = color()
