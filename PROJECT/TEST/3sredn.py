		...
		if sym1 in data and sym2 in data:
			if data[sym1]['dat'] != None and data[sym2]['dat'] != None:
				if data[sym1]['tmstp'][2] and data[sym2]['tmstp'][2]:
					...
					sredn12 = scie.getshlifmed_easy((Ask1 + Bid1) / 2, sym1+'2', persr*2)
					sredn22 = scie.getshlifmed_easy((Ask2 + Bid2) / 2, sym2+'2', persr*2)

					# ***********************************************
					# *************************************************
					# /**********************************************

					if sredn12 != None and sredn22 != None:

						Asksp = 100 * Ask1 / sredn1 - 100 * Bid2 / sredn2
						Bidsp = 100 * Bid1 / sredn1 - 100 * Ask2 / sredn2

						Asksp2 = 100 * Ask1 / sredn12 - 100 * Bid2 / sredn22
						Bidsp2 = 100 * Bid1 / sredn12 - 100 * Ask2 / sredn22

						So = scie.getshlifmed_easy(max(abs(Asksp), abs(Bidsp)), sym1 + 'So', persr * kperso)

						So2 = scie.getshlifmed_easy(max(abs(Asksp2), abs(Bidsp2)), sym1 + 'So2', persr*2 * kperso)
						...
						if So2 != None:
							...
							paintdict[sym1]['asksspread2'].append(Asksp2)
							paintdict[sym1]['bidsspread2'].append(Bidsp2)
							# ************************************************
							SoAsk2 = So2
							SoBid2 = 0 - So2
							...
							if  flagbuyvih and Bidsp > SoAsk * ksovih and Bidsp2 > SoAsk2 * ksovih  and totrade:  # Bidsp>SoAsk
								timebuyvih=tme
							...
							if flagsellvih and Asksp < SoBid * ksovih and Asksp2 < SoBid2 * ksovih and  totrade:  # Asksp<SoBid
								timesellvih=tme
							...
							# покупка
							if flagbuy and Asksp < SoBid * ksovh and Asksp2 < SoBid2 * ksovh and  So > minkso and totrade:  # and (So1-So2) >So/2:
							...
							# продажа
							if flagsell and Bidsp > SoAsk * ksovh and Bidsp2 > SoAsk2 * ksovh and   So > minkso and totrade:  # and (So1-So2) >So/2:
...
if showspread:
	color = get_color()
	fig = px.line(
		title=f"1 n SPREAD {sym1} {sym2}   persr={persr}   kperso={kperso}  ksovh ={ksovh}  ksovih ={ksovih}   minkso={minkso}   ")
	...
	clr = color()
	fig.add_scatter(x=paintdict[sym1]['ixes'], y=paintdict[sym1]['asksspread2'], line_color=clr, name=sym1 + " - " + sym2 + ' ask2')
	fig.add_scatter(x=paintdict[sym1]['ixes'], y=paintdict[sym1]['bidsspread2'], line_color=clr, name=sym1 + " - " + sym2 + ' bid2')
	clr = color()
...
color = get_color()
fig = px.line(
	title=f"1 REZ {sym1} {sym2}  persr={persr}   kperso={kperso}  ksovh ={ksovh}  ksovih ={ksovih}   minkso={minkso}   zaderzka={zaderzka} ")
clr = color()
